...
import { ArrowLeft, ArrowRight, Maximize, FileText, File, FileSpreadsheet, FileImage, FileArchive, FileBox } from 'lucide-react';
...
const PDFCarouselViewer = ({
  ...
  useEffect(() => {
    const pedimentoIndex = documentFiles.findIndex(file => 
      file.name.toLowerCase().includes('pedimento') || 
      (file.documentType && file.documentType.toLowerCase().includes('pedimento'))
    );
  ...
  const getDocumentIcon = (file: CustomGlossFileTable) => {
    ...
    if (
      ...
      (file.url.toLowerCase().endsWith('.pdf') || file.url.toUpperCase().endsWith('.PDF'))
    ) {
    ...
    if (
      ...
      return <FileBox className="h-5 w-5 mr-2 text-yellow-600" />;
    }
    ...
    if (
      ...
      return <FileArchive className="h-5 w-5 mr-2 text-amber-500" />;
    }
  ...
  const renderDocumentList = () => {
    return (
      <div className="h-[70vh] w-64 overflow-y-auto rounded-l-xl border-r border-gray-200 bg-gray-50">
        ...
        <div className="space-y-1 p-2">
          {documentFiles.map((file, index) => (
            <div
              key={file.id}
              className={cn(
                "flex items-center cursor-pointer rounded-md p-2 text-sm transition-all",
  ...
  if (documentFiles.length === 0) {
    return (
      <div className="flex h-40 w-full items-center justify-center rounded-lg border border-dashed border-gray-300">
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      {importerName && (
        <h2 className="text-xl font-bold text-gray-800">{importerName}</h2>
      )}
      ...
      >
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-center space-x-2 border-t border-gray-100 bg-white px-2 py-2">
            <Button
              variant="outline"
