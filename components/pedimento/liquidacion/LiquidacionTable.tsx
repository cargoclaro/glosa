    ...
    <div className="grid grid-cols-12 gap-0">
        ...
        <div className="grid grid-cols-3 gap-0">
          ...
          {liquidacion.conceptos.map((concepto, index) => (
            <React.Fragment key={index}>
              <div className="pedimento-cell pedimento-value">{concepto.concepto}</div>
              <div className="pedimento-cell pedimento-value text-center">{concepto.fp}</div>
              <div className="pedimento-cell pedimento-value text-right">{formatNumber(concepto.importe)}</div>
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <div className="col-span-3">
        ...
        <div className="grid grid-cols-2 gap-0">
          ...
          <div className="pedimento-cell pedimento-value text-right">{formatNumber(liquidacion.totales.efectivo)}</div>
        </div>
        <div className="grid grid-cols-2 gap-0">
          <div className="pedimento-cell pedimento-label">OTROS</div>
          <div className="pedimento-cell pedimento-value text-right">{formatNumber(liquidacion.totales.otros)}</div>
        </div>
        <div className="grid grid-cols-2 gap-0">
          <div className="pedimento-cell pedimento-label">TOTAL</div>
          <div className="pedimento-cell pedimento-value text-right">{formatNumber(liquidacion.totales.total)}</div>
        </div>
      </div>
    </div>
  );
