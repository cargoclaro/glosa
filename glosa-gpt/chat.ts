...
async function main() {
  const response = await openai.responses.create({
    ...
    input:
      'Aparte de pedimentos sencillos y consolidados, que otros tipos de pedimentos existen? No me refiero a las claves de pedimento distintas del anexo 22, sino pedimentos cualitativamente distintos.',
    tools: [
      {
        type: 'file_search',
  ...
  if (response.output && response.output.length > 0) {
    ...
    if (messageOutput?.content && searchCall?.results) {
      const contentWithCitations = messageOutput.content.find(
      ...
      if (contentWithCitations && contentWithCitations.annotations) {
        ...
        contentWithCitations.annotations.forEach((annotation: any, index) => {
          if (annotation.type === 'file_citation') {
            ...
            const citedResult = searchCall.results.find(
              (result) => result.file_id === citation.file_id
            );
