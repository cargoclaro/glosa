...
export type CartaRegla318 = z.infer<typeof cartaRegla318Schema>;

export const cartaRegla318Schema = z.object({
  document_info: z
    .object({
      referencia: z
        .string()
      ...
      fecha: z
        .string()
        .describe('Fecha de emisión del documento en formato YYYY-MM-DD.'),
      destinatario: z
        .string()
  ...
  factura: z
    .object({
      conocimiento_maritimo_aereo: z
        ...
        .optional(),
      numero_factura: z.string().describe('Número de la factura asociada.'),
      fecha_factura: z
        .string()
        .describe('Fecha de emisión de la factura en formato YYYY-MM-DD.'),
      contenedores: z
        .array(z.string())
        .describe('Lista de números de contenedores, si aplica.')
        .optional(),
      valor_factura: z.number().describe('Valor total de la factura.'),
    })
    .describe('Detalles relacionados con la factura asociada.'),
  importador_exportador: z
    .object({
      nombre: z.string().describe('Nombre o razón social del importador.'),
      domicilio: z.string().describe('Dirección completa del importador.'),
      rfc: z.string().describe('Identificador fiscal del importador (RFC)'),
    })
  ...
  mercancias: z
    .array(
      z.object({
        ...
        unidad: z
          .string()
          .describe("Unidad de medida de la mercancía, por ejemplo, 'pieza'."),
        precio_unitario: z
          .number()
          .describe(
        ...
        precio_total: z
          .number()
          .describe('Precio total de la mercancía en la moneda especificada.'),
        moneda: z.string().describe("Moneda utilizada, por ejemplo, 'USD'."),
      })
    )
  ...
  representante_legal: z
    .object({
      ...
      empresa: z.string().describe('Nombre de la empresa que representa.'),
      direccion: z
        .string()
        .describe('Dirección completa de la empresa del representante legal.'),
    })
