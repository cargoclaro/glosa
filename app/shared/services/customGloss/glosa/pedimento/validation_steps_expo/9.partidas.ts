...
export async function validateCoherenciaUMC(pedimento: Pedimento, cfdi?: Cfdi) {
  ...
  const validation = {
    ...
    description: "Verificación de que las unidades de medida comercial y sus cantidades declaradas en el pedimento coincidan con las del CFDI",
    prompt: "Los campos UMC y cantidad UMC deben coincidir con los valores en el CFDI.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
...
export async function validateCoherenciaPeso(pedimento: Pedimento, cfdi?: Cfdi) {
  ...
  const validation = {
    ...
    description: "Verificación de que el peso total de las partidas coincida con el peso bruto declarado en el pedimento y con el peso declarado en el CFDI",
    prompt: "El peso de las partidas debe coincidir con el peso bruto del pedimento y el peso declarado en el CFDI.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
...
export async function validateCalculoDTA(pedimento: Pedimento) {
  ...
  const validation = {
    ...
    description: "Verificación de que el cálculo del prorrateo y DTA coincida con los valores declarados en el pedimento",
    prompt: "El prorrateo y el DTA calculados deben coincidir con los declarados. En el caso de DTA en cuota fija, divide el DTA entre el número de secuencias.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
...
export async function validateCalculoContribuciones(pedimento: Pedimento, cfdi?: Cfdi) {
  ...
  const validation = {
    ...
    description: "Verificación de que los valores declarados en las contribuciones coincidan con los calculados según la normativa",
    prompt: "Los valores de precio pagado, precio unitario, valor aduana, IGI y DTA deben coincidir con los calculados. En exportación, verificar que el valor comercial coincida con el total del CFDI.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
...
export async function validatePermisosIdentificadores(pedimento: Pedimento) {
  ...
  const validation = {
    ...
    description: "Verificación de que los permisos e identificadores declarados en el pedimento sean válidos para exportación",
    prompt: "Los permisos e identificadores en el pedimento deben existir en Taxfinder y ser apropiados para una operación de exportación.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
...
export async function validateRegulacionesArancelarias(pedimento: Pedimento) {
  ...
  const validation = {
    ...
    description: "Verificación de regulaciones arancelarias aplicables a la mercancía en exportación",
    prompt: "Verifica si existen regulaciones arancelarias que apliquen a la mercancía en un contexto de exportación.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
...
export async function validateRegulacionesNoArancelarias(pedimento: Pedimento) {
  ...
  const validation = {
    ...
    description: "Verificación de regulaciones no arancelarias aplicables a la mercancía en exportación, como permisos de SEMARNAT, COFEPRIS u otras dependencias",
    prompt: "Verifica si existen regulaciones no arancelarias que apliquen a la mercancía en un contexto de exportación, como permisos de SEMARNAT, COFEPRIS u otras dependencias para productos de exportación.",
    contexts: {
      [CustomGlossTabContextType.PROVIDED]: {
        pedimento: {
          data: [
