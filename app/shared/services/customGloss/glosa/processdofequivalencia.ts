...
async function getNoteIdByDate(date: string): Promise<string> {
  ...
  const targetNote = data.NotasMatutinas.find((note) =>
    regex.test(note.titulo)
  );
...
function parseEquivalencias(
  html: string
): Array<{ pais: string; moneda: string; valor: string }> {
  const $ = load(html);
...
async function main(date = '07-01-2025') {
  try {
