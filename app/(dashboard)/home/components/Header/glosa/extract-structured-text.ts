...
import {
  cfdiSchema,
  facturaSchema,
  listaDeFacturasSchema,
  reportesRemesaSchema,
} from './schemas';
import type { ClassifiedDocumentSet, StructuredDocumentSet } from './types';
...
export async function extractStructuredText(
  ...
  const cfdisData = await Promise.all(
    cfdis.map(async ({ originalFile, ufsUrl }) => {
      const xmlData = await originalFile.text();
      const cfdiData = cfdiSchema.safeParse(xmlParser.parse(xmlData, true));
      if (!cfdiData.success) {
        throw new Error(
          `Error parsing cfdi: ${cfdiData.error.message}. XML URL: ${ufsUrl}`
        );
      }
  ...
  const { object: listaDeFacturasData } = await generateObject({
    ...
    system:
      'Eres un experto en extraccion y estructuracion de datos de documentos aduaneros.',
  ...
  const reporteEDocumentRemesaConsolidadoPdfPages = await extractPdfPages(
    await reporteEDocumentRemesaConsolidado.originalFile.arrayBuffer()
  );
  const reporteEDocumentRemesaConsolidadoData = await Promise.all(
    reporteEDocumentRemesaConsolidadoPdfPages.map(async (base64Page, index) => {
  ...
  return {
    ...
    reporteRemesa: reporteEDocumentRemesaConsolidadoData.flatMap(
      (reportArray) =>
